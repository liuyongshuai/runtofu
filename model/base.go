...
//获取单行信息
func (m *BaseModel) FetchRow(cond map[string]interface{}) (map[string]goUtils.ElemType, error) {
	ret := make(map[string]goUtils.ElemType)
...
func (m *BaseModel) FetchList(cond map[string]interface{}, page int, pagesize int, orderby string) []map[string]goUtils.ElemType {
	if page <= 0 {
		page = 1
	}
	if pagesize < 1 {
	...
	where := ""
	cd, param := goUtils.FormatCond(cond, "AND")
	if len(param) > 0 {
